...
class ThinkImage {
    ...
    static function getThinkImageInfo($img) {
    ...
    static public function water($source, $water, $savename=null, $alpha=80) {
        ...
        //图片信息
        $sInfo = self::getThinkImageInfo($source);
        $wInfo = self::getThinkImageInfo($water);
        ...
        $sThinkImage = $sCreateFun($source);
        $wCreateFun = "imagecreatefrom" . $wInfo['type'];
        $wThinkImage = $wCreateFun($water);

        //设定图像的混色模式
        imagealphablending($wThinkImage, true);
        ...
        //生成混合图像
        self::imagecopymerge_alpha($sThinkImage, $wThinkImage, $posX, $posY, 0, 0, $wInfo['width'], $wInfo['height'], $alpha);

        //输出图像
        $ThinkImageFun = 'ThinkImage' . $sInfo['type'];
        //如果没有给出保存文件名，默认为原图像名
        if (!$savename) {
            $savename = $source;
            @unlink($source);
        ...
        //保存图像
        $ThinkImageFun($sThinkImage, $savename);
        imagedestroy($sThinkImage);
    }

    function showImg($imgFile, $text='', $x='10', $y='10', $alpha='50') {
        //获取图像文件信息
        //2007/6/26 增加图片水印输出，$text为图片的完整路径即可
        $info = ThinkImage::getThinkImageInfo($imgFile);
        if ($info !== false) {
            $createFun = str_replace('/', 'createfrom', $info['mime']);
            $im = $createFun($imgFile);
            if ($im) {
                $ThinkImageFun = str_replace('/', '', $info['mime']);
                //水印开始
                if (!empty($text)) {
                    $tc = imagecolorallocate($im, 0, 0, 0);
                    if (is_file($text) && file_exists($text)) {//判断$text是否是图片路径
                        // 取得水印信息
                        $textInfo = ThinkImage::getThinkImageInfo($text);
     ...
     * @return void
     */
    static function thumb($image, $thumbname, $type='', $maxWidth=500, $maxHeight=500, $interlace=true) {
        // 获取原图信息
        $info = ThinkImage::getThinkImageInfo($image);
        if ($info !== false) {
            $srcWidth = $info['width'];
            ...
            // 复制图片
            if (function_exists("ThinkImageCopyResampled"))
    ...
    static function buildString($string, $rgb=array(), $filename='', $type='png', $disturb=1, $border=true) {
        ...
        ThinkImage::output($im, $type, $filename);
    }
     ...
     * @return string
     */
    static function buildThinkImageVerify($length=4, $mode=1, $type='png', $width=48, $height=22, $verifyName='verify') {
    ...
    static function showASCIIImg($image, $string='', $type='') {
        ...
        if ($info !== false) {
            ...
            unset($info);
            // 载入原图
            $createFun = 'ThinkImageCreateFrom' . ($type == 'jpg' ? 'jpeg' : $type);
            $im = $createFun($image);
            $dx = imagesx($im);
            $dy = imagesy($im);
            $i = 0;
    ...
    '1001110', '1010000', '1000100', '1001000', '1110100');
        ...
        $bars.=$ends;
        /* Generate the Barcode ThinkImage */
        if ($type != 'gif' && function_exists('imagecreatetruecolor')) {
            $im = imagecreatetruecolor($lw * 95 + 30, $hi + 30);
        } else {
            $im = imagecreate($lw * 95 + 30, $hi + 30);
        }
        $fg = ThinkImageColorAllocate($im, 0, 0, 0);
        $bg = ThinkImageColorAllocate($im, 255, 255, 255);
        ThinkImageFilledRectangle($im, 0, 0, $lw * 95 + 30, $hi + 30, $bg);
        $shift = 10;
        for ($x = 0; $x < strlen($bars); $x++) {
            ...
            ThinkImageFilledRectangle($im, ($x * $lw) + 15, 5, ($x + 1) * $lw + 14, $hi + 5 + $sh, $color);
        }
        /* Add the Human Readable Label */
        ThinkImageString($im, 4, 5, $hi - 5, $code[0], $fg);
        for ($x = 0; $x < 5; $x++) {
            ThinkImageString($im, 5, $lw * (13 + $x * 6) + 15, $hi + 5, $code[$x + 1], $fg);
            ThinkImageString($im, 5, $lw * (53 + $x * 6) + 15, $hi + 5, $code[$x + 6], $fg);
        }
        ThinkImageString($im, 4, $lw * 95 + 17, $hi - 5, $code[11], $fg);
    ...
    static function output($im, $type='png', $filename='') {
        ...
        $ThinkImageFun = 'image' . $type;
        if (empty($filename)) {
            $ThinkImageFun($im);
        } else {
            $ThinkImageFun($im, $filename);
        }
        imagedestroy($im);
    }
